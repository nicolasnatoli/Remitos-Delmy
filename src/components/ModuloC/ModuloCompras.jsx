...
function buildIdx(art){
  ...
  for(const[cod,a]of Object.entries(art)){
    ...
    if(cpN&&cpN!==cp){if(!idx[cpN])idx[cpN]=[];idx[cpN].push(cod);}
  }
  return idx;
}
...
function buscar(desc,codDoc,prov,famF,catF,marcaF,q,art){
  ...
  for(const[cod,a]of Object.entries(art)){
    ...
    if(codN&&cpN){
      ...
      else if(cpN.includes(codN)||codN.includes(cpN)){score+=18;type='prim';}
    }
...
function enriquecerLinea(cod,cant,precioDoc,descDoc,db,idx){
  const codI=cruzar(cod,db.art,idx)||cod;
  const a=db.art[codI]||{desc:descDoc||'',codp:cod,prov:'',fam:'',cat:'',costoReal:0,pvMin:0,mostrador:0};
  ...
  return{
    cod:codI,codp:cod,desc:a.desc||descDoc||'',prov:a.prov||'',fam:a.fam||'',cat:a.cat||'',
    ...
    reconocido:!!(db.art[codI]||codI!==cod),
...
export default function ModuloCompras(){
  ...
  const codpIdx=useMemo(()=>buildIdx(db.art),[db.art]);
  ...
  const reloadDB=useCallback(()=>{loadDB().then(f=>{setDb(f);setDbReady(true);});},[]);
  ...
  const transicion=useCallback((id,est,data,extra={})=>{
    ...
    const entrada={estado:est,ts,label:nowLabel(),usuario:'Operario',...extra,desdePrev:mins};
  ...
  const importarDesdeStock=useCallback(()=>{
    ...
    const listaItems=sh?.listaItems||getListaCompra().items||{};
    const prov=sh?.prov||'';
    if(!planC&&!Object.keys(listaItems).length){alert('En Stock+: tildá artículos o completá "A Comprar" y presioná "→ Compras".');return;}
    const plan=planC?expandPlan(planC):{};
    // Combinar plan + lista
    const todos={};
  ...
  const procesarDoc=useCallback(async(file)=>{
    ...
    if(ext==='xlsx'||ext==='xls'){
      ...
      try{
        ...
        if(parsed.proveedor)setOCdata(prev=>({...prev,meta:{...prev.meta,proveedor:parsed.proveedor,documento:parsed.nDocumento||prev.meta.documento}}));
        aplicarDocLineas((parsed.lineas||[]).map(l=>({cod:l.cod,desc:l.desc||'',cant:Number(l.cant)||0,precio:Number(l.precioUnit)||0})));
  ...
  const selectOC=(id)=>{setOCact(id);const d=lsGet('dm_oc_v3_'+id,null);if(d){setOCdata({meta:d.meta||{},lineas:d.lineas||[]});}setEtC('validacion');};
  const deleteOC=(id)=>{if(!window.confirm('¿Eliminar OC?'))return;setOCS(prev=>{const n=prev.filter(x=>x!==id);lsSet(SK.ocs,n);return n;});try{localStorage.removeItem('dm_oc_v3_'+id);}catch{}if(OCact===id){setOCact(null);setOCdata({meta:{},lineas:[]});setEtC('carga');}};

  const exportarNuevos=()=>{
    const nList=lsGet(SK.nuevos,[]);if(!nList.length){alert('Sin artículos nuevos');return;}
    const rows=[['Código','Cód.Prov','Descripción','Familia','Categ.','Marca','Costo Real','PV Mín.','Mostrador','Fecha Alta']];
    nList.forEach(n=>rows.push([n.cod,n.codp,n.desc,n.fam,n.cat,n.marca,n.costoReal,n.pvMin,n.mostrador,n.fechaAlta]));
  ...
  const exportarOC=()=>{
    ...
    const rows=[['Código','Cód.Prov','Descripción','Familia','Cant.OC','Precio Doc.','Costo Real','PV Mín.','Mostrador','Subtotal','Stk Cen','Stk Sol','Stk Var','V.Sem','V.Quin','V.Mes','→Central','→Solano','→Varela','→DP']];
    OCdata.lineas.forEach(l=>{const dp=Math.max(0,l.cantOC-(l.dc||0)-(l.d1||0)-(l.d3||0));rows.push([l.cod,l.codp,l.desc,l.fam,l.cantOC,l.precioDoc||0,l.costoReal||0,l.pvMin||0,l.mostrador||0,l.cantOC*(l.precioDoc||0),l.stkDMCN||0,l.stkDM01||0,l.stkDM03||0,l.vs||0,l.vq||0,l.vm||0,l.dc||0,l.d1||0,l.d3||0,dp]);});
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',height:'calc(100vh - 56px)',background:C.bg}}>
      <div style={{background:C.p2,borderBottom:`1px solid ${C.b1}`,display:'flex',padding:'0 14px',flexShrink:0,alignItems:'center',gap:8}}>
        ...
        {!dbReady&&<span style={{fontSize:9,color:C.mut}}>Cargando DB...</span>}
        {dbReady&&<span style={{fontSize:9,color:Object.keys(db.art).length>0?C.teal:C.red}}>{Object.keys(db.art).length>0?`✓ ${fn(Object.keys(db.art).length)} arts en DB`:'⚠ Sin DB — cargar en Stock+'}</span>}
        <div style={{marginLeft:'auto',display:'flex',gap:6}}>
          ...
          {nNuevos>0&&<button onClick={exportarNuevos} style={Btn(C.ora,'rgba(251,146,60,.08)')}>↓ Nuevos ({nNuevos})</button>}
        </div>
      </div>

      {modal.open&&<ModalArt modal={modal} setModal={setModal} linea={OCdata.lineas[modal.idx]} db={db} codpIdx={codpIdx} onAsignar={asignarArt} onNuevo={confirmarNuevo} />}
      ...
      <div style={{flex:1,overflow:'auto',padding:12}}>
        ...
        {OCS.length>0&&(
          <div style={{background:C.panel,border:`1px solid ${C.b1}`,borderRadius:5,overflow:'hidden',marginBottom:10}}>
            ...
            <div style={{maxHeight:72,overflowY:'auto'}}>
              {OCS.map(id=>{const d=lsGet('dm_oc_v3_'+id,null);if(!d)return null;const cfg=ESTADOS[d.meta?.estado]||null;return(
        ...
        <div style={{background:C.panel,border:`1px solid ${C.b1}`,borderTop:'none',borderRadius:'0 0 5px 5px',padding:14}}>
          {etC==='carga'&&<EtCarga OCdata={OCdata} setOCdata={setOCdata} importarDesdeStock={importarDesdeStock} procesarDoc={procesarDoc} onNext={()=>setEtC('validacion')} saveOC={saveOC} OCact={OCact} />}
          {etC==='validacion'&&<EtValidacion OCdata={OCdata} setOCdata={setOCdata} db={db} dbReady={dbReady} procesarDoc={procesarDoc} saveOC={saveOC} OCact={OCact} abrirModal={abrirModal} onBack={()=>setEtC('carga')} onNext={()=>setEtC('distribucion')} />}
          {etC==='distribucion'&&<EtDistribucion OCdata={OCdata} setOCdata={setOCdata} saveOC={saveOC} OCact={OCact} onBack={()=>setEtC('validacion')} onNext={()=>setEtC('confirmar')} />}
          {etC==='confirmar'&&<EtConfirmar OCdata={OCdata} saveOC={saveOC} OCact={OCact} transicion={transicion} onBack={()=>setEtC('distribucion')} />}
        </div>
      </div>
...
function EtCarga({OCdata,setOCdata,importarDesdeStock,procesarDoc,onNext,saveOC,OCact}){
  ...
  return(
    <div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12,marginBottom:12}}>
        {[
          {t:'DESDE STOCK+',info:<Alrt cls="ok">✓ Tildá artículos en Stock+ y presioná → Compras</Alrt>,btn:<button onClick={importarDesdeStock} style={{...Btn(C.acc,'rgba(240,192,64,.1)'),width:'100%',fontWeight:600}}>← Importar desde Stock+</button>},
          {t:'PLANILLA EXCEL',info:<Alrt cls="info">Exportada desde Stock+ o del proveedor</Alrt>,btn:<button onClick={()=>fileRef.current.click()} style={{...Btn(),width:'100%'}}>📋 Cargar .xlsx</button>},
          {t:'FACTURA / REMITO',info:<Alrt cls="info">PDF, imagen o Excel — lectura con IA</Alrt>,btn:<button onClick={()=>fileRef.current.click()} style={{...Btn(),width:'100%'}}>📄 Subir documento</button>},
        ].map((op,i)=>(
          <div key={i}>
            <div style={{fontSize:9,color:C.mut,marginBottom:6}}>{op.t}</div>
            {op.info}{op.btn}
          </div>
        ))}
      </div>
      {hasOC&&<Alrt cls="ok">✓ OC activa: {OCdata.lineas.length} artículos · {OCdata.meta.proveedor||'(sin prov)'} · {OCdata.meta.origen||'manual'}</Alrt>}
...
function EtValidacion({OCdata,setOCdata,db,dbReady,procesarDoc,saveOC,OCact,abrirModal,onBack,onNext}){
  const fileRef=useRef();
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:6,marginBottom:8}}>
        {[{l:'LÍNEAS',v:OCdata.lineas.length,c:C.txt},{l:'RECONOCIDAS',v:rec,c:C.green},{l:'NO RECONOCIDAS',v:noRec,c:noRec>0?C.red:C.mut},{l:'PRECIO ↑',v:suben,c:suben>0?C.red:C.mut},{l:'PRECIO ↓',v:bajan,c:bajan>0?C.green:C.mut}].map(k=>(
          <div key={k.l} style={{background:C.p2,border:`1px solid ${C.b1}`,borderRadius:4,padding:'6px 9px'}}>
            <div style={{fontSize:8,color:C.mut,letterSpacing:'.07em',textTransform:'uppercase',marginBottom:2}}>{k.l}</div>
            <div style={{fontFamily:'Syne,sans-serif',fontSize:16,fontWeight:700,color:k.c}}>{k.v}</div>
          </div>
        ))}
      </div>

      {dbReady&&Object.keys(db.art).length===0&&<Alrt cls="err">⚠ Base vacía — cargá FormatoProveedores en Stock+ y hacé clic en ↺ DB</Alrt>}
      {noRec>0&&<Alrt cls="warn">⚠ {noRec} artículo(s) sin reconocer — click en "Resolver →"</Alrt>}
      {!conFac&&<Alrt cls="info">Sin factura — los precios muestran el Costo Real de la base. Subí la factura para comparar.</Alrt>}
      {suben>0&&<Alrt cls="err">↑ {suben} artículo(s) con precio superior al Costo Real</Alrt>}
      {bajan>0&&<Alrt cls="ok">↓ {bajan} artículo(s) con precio inferior al Costo Real</Alrt>}
      ...
      <div style={{overflowX:'auto',background:C.p2,border:`1px solid ${C.b1}`,borderRadius:5}}>
        <table style={{borderCollapse:'collapse',width:'100%',minWidth:1100}}>
          <thead>
            <tr>
              {[
                ['CÓD.DOC',C.blue],['CÓD.BASE',C.teal],['DESCRIPCIÓN',C.mut],['FAM.',C.mut],
                ['CEN',C.teal],['SOL',C.blue],['VAR',C.green],['TOTAL STK',C.mut],
                ['V.SEM',C.mut],['V.QUIN',C.mut],['V.MES',C.mut],
                ['CANT.',C.txt],['PRECIO DOC.',C.acc],['COSTO REAL',C.mut],['MOSTRADOR',C.blue],['PV MÍN.',C.vio],['SUBTOTAL',C.acc],
                ['DIFF',C.mut],['ACCIÓN',C.mut]
              ].map(([h,c],i)=>(
                <th key={i} style={{fontSize:8,color:c,padding:'4px 6px',borderBottom:`1px solid ${C.b1}`,whiteSpace:'nowrap',textTransform:'uppercase',letterSpacing:'.05em',textAlign:i>3?'right':'left',background:C.p2}}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {OCdata.lineas.map((l,i)=>{
              ...
              const rowBg=!l.reconocido?'rgba(248,113,113,.04)':diff!==null&&diff>0&&!l.aprobado?'rgba(248,113,113,.02)':'transparent';
              let accion=null;
              ...
              else if(diff!==null&&diff>0&&!l.aprobado)accion=<div style={{display:'flex',gap:2}}><button onClick={()=>aprobar(i,true)} style={{...Btn(C.green,'rgba(74,222,128,.1)'),fontSize:9,padding:'2px 5px'}}>✓</button><button onClick={()=>aprobar(i,false)} style={{...Btn(C.red,'rgba(248,113,113,.1)'),fontSize:9,padding:'2px 5px'}}>✗</button></div>;
              ...
              return(
                <tr key={i} style={{background:rowBg}}>
                  ...
                  {td(<NumIn value={l.cantOC} onChange={v=>updLinea(i,'cantOC',v)} color={C.txt} />,{textAlign:'right',padding:'3px 4px'})}
            ...
            {/* Totales */}
            <tr style={{background:'rgba(240,192,64,.04)',fontWeight:600}}>
              <td colSpan={11} style={{padding:'5px 6px',fontSize:9,color:C.mut,textAlign:'right',borderTop:`1px solid ${C.b1}`}}>TOTALES</td>
              <td style={{padding:'5px 6px',textAlign:'right',borderTop:`1px solid ${C.b1}`,fontSize:10}}>{fn(OCdata.lineas.reduce((s,l)=>s+l.cantOC,0))}</td>
              <td style={{padding:'5px 6px',textAlign:'right',borderTop:`1px solid ${C.b1}`,fontSize:10,color:C.acc}}></td>
              <td colSpan={3} style={{padding:'5px 6px',borderTop:`1px solid ${C.b1}`}}></td>
              <td style={{padding:'5px 6px',textAlign:'right',borderTop:`1px solid ${C.b1}`,fontSize:10,color:C.acc,fontWeight:700}}>${fn(OCdata.lineas.reduce((s,l)=>s+l.cantOC*(l.precioDoc||0),0))}</td>
  ...
  return(
    <div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:7,marginBottom:10}}>
        {[{l:'TOTAL OC',v:fn(totOC),c:C.txt},{l:'→ CENTRAL',v:fn(totCen),c:C.teal},{l:'→ SOLANO',v:fn(totSol),c:C.blue},{l:'→ VARELA',v:fn(totVar),c:C.green}].map(k=>(
      ...
      <div style={{overflowX:'auto',background:C.p2,border:`1px solid ${C.b1}`,borderRadius:5,marginBottom:10}}>
        <table style={{borderCollapse:'collapse',width:'100%'}}>
          <thead><tr>
            {['CÓDIGO','DESCRIPCIÓN','CANT.OC','COSTO UNIT.','SUBTOTAL','→CENTRAL','→SOLANO','→VARELA','→DP','DIFF'].map((h,i)=>(
              <th key={i} style={{fontSize:8,color:h==='→CENTRAL'?C.teal:h==='→SOLANO'?C.blue:h==='→VARELA'?C.green:h==='→DP'?C.vio:C.mut,padding:'5px 6px',background:C.p2,borderBottom:`1px solid ${C.b1}`,textTransform:'uppercase',letterSpacing:'.05em',textAlign:i>1?'right':'left'}}>{h}</th>
            ))}
          </tr></thead>
          <tbody>
            {OCdata.lineas.map((l,i)=>{
            ...
            <tr style={{fontWeight:600}}>
              {['','','','','',fn(totCen),fn(totSol),fn(totVar),'',''].map((v,i)=>(
                <td key={i} style={{padding:'5px 6px',borderTop:`1px solid ${C.b1}`,fontSize:9,textAlign:'right',color:[5,6,7].includes(i)?[C.teal,C.blue,C.green][i-5]:C.mut}}>{v}</td>
              ))}
            </tr>
          </tbody>
        </table>
...
// ─── Modal ────────────────────────────────────────────────────────────────────
function ModalArt({modal,setModal,linea,db,codpIdx,onAsignar,onNuevo}){
  if(!linea)return null;
  const freq=getFreq(linea.prov,db.art);
  const res=buscar(linea.desc,linea.codp||linea.cod,linea.prov,modal.selFam,modal.selCat,modal.selMarca,modal.busqQ,db.art);
  ...
  return(
    <div style={{background:'rgba(0,0,0,.8)',padding:12,flexShrink:0}}>
      <div style={{background:C.panel,border:`1px solid ${C.b1}`,borderRadius:6,overflow:'hidden',maxHeight:'75vh',display:'flex',flexDirection:'column'}}>
        <div style={{background:'rgba(240,192,64,.06)',borderBottom:'1px solid rgba(240,192,64,.2)',padding:'10px 14px',display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:10,flexShrink:0}}>
          <div>
            <div style={{fontSize:11,fontWeight:500,color:C.acc}}>
              ...
              <span style={{fontSize:9,color:nArt>0?C.teal:C.red,marginLeft:10}}>{nArt>0?`${fn(nArt)} arts en DB`:'⚠ Sin DB'}</span>
            </div>
          ...
          <div style={{display:'flex',flexDirection:'column',overflow:'hidden',flex:1}}>
            <div style={{padding:'10px 14px',flexShrink:0}}>
              <div style={{marginBottom:8}}>
                ...
                <input placeholder="Código, descripción, marca..." value={modal.busqQ} onChange={e=>setModal(m=>({...m,busqQ:e.target.value}))} style={{...IS,fontSize:11}} autoFocus />
              </div>
              <div style={{fontSize:9,color:C.mut,marginBottom:7,display:'flex',gap:12,flexWrap:'wrap'}}>
              ...
              {freq.fams.length>0&&(
                <div style={{marginBottom:5}}>
                  <div style={{fontSize:8,color:C.mut,marginBottom:3,textTransform:'uppercase',letterSpacing:'.06em'}}>FAMILIA</div>
                  <div>{freq.fams.map(f=><span key={f} onClick={()=>toggle('selFam',f)} style={{display:'inline-flex',alignItems:'center',padding:'2px 8px',borderRadius:3,fontSize:9,border:`1px solid ${modal.selFam===f?C.acc:C.b1}`,cursor:'pointer',margin:'1px 2px',background:modal.selFam===f?'rgba(240,192,64,.15)':'transparent',color:modal.selFam===f?C.acc:C.txt}}>{f}</span>)}</div>
                </div>
              )}
              {freq.cats.length>0&&(
                <div style={{marginBottom:5}}>
                  <div style={{fontSize:8,color:C.mut,marginBottom:3,textTransform:'uppercase',letterSpacing:'.06em'}}>CATEGORÍA</div>
                  <div>{freq.cats.map(c=><span key={c} onClick={()=>toggle('selCat',c)} style={{display:'inline-flex',alignItems:'center',padding:'2px 8px',borderRadius:3,fontSize:9,border:`1px solid ${modal.selCat===c?C.acc:C.b1}`,cursor:'pointer',margin:'1px 2px',background:modal.selCat===c?'rgba(240,192,64,.15)':'transparent',color:modal.selCat===c?C.acc:C.txt}}>{c}</span>)}</div>
                </div>
              )}
              {freq.marcas.length>0&&(
                <div style={{marginBottom:6}}>
                  <div style={{fontSize:8,color:C.mut,marginBottom:3,textTransform:'uppercase',letterSpacing:'.06em'}}>MARCA</div>
                  <div>{freq.marcas.map(m=><span key={m} onClick={()=>toggle('selMarca',m)} style={{display:'inline-flex',alignItems:'center',padding:'2px 8px',borderRadius:3,fontSize:9,border:`1px solid ${modal.selMarca===m?C.acc:C.b1}`,cursor:'pointer',margin:'1px 2px',background:modal.selMarca===m?'rgba(240,192,64,.15)':'transparent',color:modal.selMarca===m?C.acc:C.txt}}>{m}</span>)}</div>
                </div>
              )}
            ...
            <div style={{flex:1,overflowY:'auto',borderTop:`1px solid ${C.b1}`}}>
              {nArt===0&&<div style={{padding:16,textAlign:'center',color:C.red,fontSize:11}}>⚠ Base vacía — hacé ↺ DB en el módulo de Compras</div>}
              {nArt>0&&res.length===0&&<div style={{padding:16,textAlign:'center',color:C.mut,fontSize:11}}>Sin resultados · probá otros filtros o buscá manualmente</div>}
              {res.map(({cod,a,type})=>(
        ...
        {modal.tab==='nuevo'&&(
          <div style={{padding:14,overflow:'auto',flex:1}}>
            <Alrt cls="info">Base de artículos es de solo lectura. El artículo se agrega a la lista de importación al sistema.</Alrt>
